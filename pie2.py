...
# 加载 DePlot 模型（只加载一次）
deplot_processor = Pix2StructProcessor.from_pretrained('D:\model\deeplot\models--google--deplot')
deplot_model = Pix2StructForConditionalGeneration.from_pretrained('D:\model\deeplot\models--google--deplot')
...
class GraphGenerator:
    ...
    def call_gpt_and_generate(self, initial_instruction: str, requirement: str, student_answer: str, image_path: Optional[str] = None, model: str = "deepseek-chat", output_format: str = "json") -> Dict:
        try:
            ...
            return self._process_response(response, output_format,image_path,student_answer, palette)
    ...
    def find_best_match_batch(self, target: str, candidates: list, student_answer, cutoff=0.85) -> Optional[str]:
        """
        ...
        # writing_context = student_answer
    ...
    def compare_and_generate_json(self, deplot_txt: str, student_txt: str, student_answer, palette, title="Generated Chart") -> Dict:
        ...
        for cat, deplot_val in deplot_data.items():
            best_match = self.find_best_match_batch(cat, list(student_data.keys()), student_answer)  # 使用批量对比
            if best_match:
    ...
    def _process_response(self, response, output_format: str, image_path, student_answer, palette) -> Dict:
        """
        响应处理方法
        """
        try:
            ...
            data = self.compare_and_generate_json(deplot_txt, student_txt, student_answer, palette)
...
In comparison to the couples, singles struggled more. Talking about people with children, single parents presented the maximum percentage of 26% amongst all the specified categories, whereas couples with children reported a comparatively lesser percentage of 15%.
